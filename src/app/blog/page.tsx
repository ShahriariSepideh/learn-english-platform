                ...
                <div className="mb-8 rounded-[2rem] border border-slate-200/80 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur-xl transition-colors duration-300 dark:border-slate-800/90 dark:bg-slate-900/90 dark:shadow-black/30">
                    <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <Link
                                href="/"
                                className="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-5 py-3 font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800"
                            >
                                خانه
                            </Link>
                        </div>
...
function BlogCard({ blog }: { blog: Blog }) {
    ...
    const difficulty = getBlogDifficulty(blog);
    ...
    return (
        <article className="overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white/90 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/60 dark:border-slate-800/90 dark:bg-slate-900/90 dark:hover:shadow-black/30">
            <div className="relative flex h-44 items-center justify-center overflow-hidden bg-slate-200 transition-colors duration-300 dark:bg-slate-800">
                {image ? (
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-gradient-to-br from-emerald-100 to-slate-200 text-emerald-700 dark:from-emerald-400/10 dark:to-slate-800 dark:text-emerald-300">
                        <ImageIcon size={32} />
                        <span className="text-sm font-black">Blog Image</span>
                    </div>
                )}

                {blog.featured && (
                    <span className="absolute right-4 top-4 inline-flex items-center gap-1 rounded-full bg-emerald-400 px-3 py-1 text-xs font-black text-slate-950 shadow-sm">
                        <Sparkles size={14} />
                        ویژه
                    </span>
                )}
            ...
            <div className="p-5">
                ...
                <p className="line-clamp-4 min-h-28 leading-7 text-slate-500 dark:text-slate-400">
                    {description}
                </p>

                <div className="mt-5 grid gap-3">
                    <InfoRow icon={<Tag size={18} />} label="دسته‌بندی" value={category} />
                    <InfoRow icon={<Feather size={18} />} label="سطح" value={difficulty} />
                    <InfoRow icon={<CalendarDays size={18} />} label="تاریخ" value={date} />
                    <InfoRow icon={<Clock3 size={18} />} label="زمان مطالعه" value={readingTime} />
                </div>
            </div>
        </article>
    );
...
function BlogsError({
    ...
    return (
        <div className="rounded-[2rem] border border-red-200 bg-red-50 p-6 text-red-700 dark:border-red-400/30 dark:bg-red-400/10 dark:text-red-300">
            <h2 className="font-black">خطا در دریافت مقاله‌ها</h2>
